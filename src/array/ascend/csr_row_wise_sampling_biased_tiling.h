#ifndef CSR_ROW_WISE_SAMPLING_BIASED_TILING_H
#define CSR_ROW_WISE_SAMPLING_BIASED_TILING_H

#include <cstdint>

// Tiling layout for the multi-core biased CSR row-wise sampling kernels
// (main sampler + per-row pick-count helper). The device block is:
// header (kTilingHeaderWords words, field order below) followed by nothing —
// per-block row-range and output-start tables travel as separate GM
// parameters (spmm pattern). Field order must match the kernels' Init.
constexpr uint32_t kTilingHeaderWords = 9;

struct CsrRowWiseSamplingBiasedTiling {
  ...
  uint32_t num_tags;        // T: number of tag buckets (<= kMaxTagCount)
};
...
// Upper bound on the number of tag buckets supported by the kernel. rem[T]
// (uint32) + bias[T] (float) state occupies at most 8KB of UB at T=1024,
// safe under the smallest per-core UB budget (192KB). Real-world tag counts
// are a few units (upstream tests: 4), so this bound is never hit in
// practice; the launcher CHECKs and reports it clearly instead of silently
// degrading.
constexpr uint32_t kMaxTagCount = 1024;

// RNG constants (xorshift32 with Knuth golden-ratio row hashing), shared
// with the uniform sampling kernel.
...
#endif  // CSR_ROW_WISE_SAMPLING_BIASED_TILING_H
