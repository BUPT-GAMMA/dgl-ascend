        ...
        for (uint32_t row = this->startRow; row < rowEnd; ++row) {
            ...
            if (rowNonZeroCount == 0) {
                ...
                for (uint32_t batchIndex = 0; batchIndex < batchCount; ++batchIndex) {
                    ...
                    ComputeBatchSum(accumUb, currentBatchSize);
                }
            }
...
private:
    ...
    __aicore__ inline void ComputeBatchSum(AscendC::LocalTensor<half> &accumBlock, uint32_t batchNnz)
    {
        AscendC::LocalTensor<half> computeBatch = featureQueue.DeQue<half>();
        for (uint32_t i = 0; i < batchNnz; ++i) {
            AscendC::Add(accumBlock, accumBlock, computeBatch[i * this->rowAlignedElems], this->N);
        }
...
extern "C" __global__ __aicore__ void spmm_sum_aiv(
    GM_ADDR featureData,
    GM_ADDR outputData,
    GM_ADDR indptrData,
    GM_ADDR indicesData,
